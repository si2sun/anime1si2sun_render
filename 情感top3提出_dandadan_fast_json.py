...
"""
    ...
    print("="*60)
    print(f"開始分析動畫: 《{ANIME_NAME_TO_ANALYZE}》")
    print("="*60)
    ...
    print("\n" + "="*60)
    print("      分析完成！最終情感熱點報告如下：")
    print("="*60 + "\n")
    
    if emotional_hotspots:
        print("--- 情感分類熱點 ---")
        print(json.dumps(emotional_hotspots, indent=4, ensure_ascii=False))

    if top_5_moments:
        print("\n--- TOP 5 彈幕密度時段 ---")
        print(json.dumps({"TOP 5 彈幕時段": top_5_moments}, indent=4, ensure_ascii=False))

    if not emotional_hotspots and not top_5_moments:
        print("非常抱歉，根據目前的設定，找不到任何熱點。")